...
- 虚线 ：依赖关系，表示一种使用关系，常见用法：一个类使用另一个类作为方法参数或者返回值
- 实线： 关联关系，常见用法：一个类把另一个类当作成员变量使用，即当作属性来使用

实线箭头 + 

- 空心菱形 ：聚合关系，两者有各自独立的生命周期，has a的关系
- 实心菱形 ：组合关系，两者有相同的生命周期，contains a的关系，关系更加强烈

### UML时序图

定义：显示对象之间的交互关系，并按照时间先后的顺序排列

建模元素：对象（Actor），生命线（Lifeline），控制焦点（Focus of control），消息（Message）等

![时序图](https://i.loli.net/2019/08/29/sLFCxTi6QWISM7O.png)

![UML类图](https://i.loli.net/2019/08/29/vsV8qIU5ahJR7uT.png)
...
**模式定义：由一个工厂对象决定创建哪一种产品类的实例，不属于GOF23种设计模式**

使用场景：

1. 创建的对象比较少
2. 只需要传入简单的参数就可以获取想要的对象，而无需关注具体创建细节
...
- Factory Method模式
- 工厂方法用于创建同一产品类型的实例，具体实例化在具体工厂实现类中完成
- 工厂方法用于创建某一产品类型（如海尔冰箱，美的冰箱，西门子冰箱等），而抽象工厂用于某一产品族（如美的冰箱，美的洗衣机，美的空调等）

**模式例子**

1. Collection 接口 iterator方法
2. UrlStreamHandlerFactory
3. slf4j中LoggerFactory的使用
